...
public class TableInfo {
	...
	private <T> void setInit() {
		...
		if (DBLogger.DEBUG) {
			DBLogger.v(SqliteUtility.TAG, String.format("类 %s 的主键是 %s", clazz.getSimpleName(), primaryKey.getColumn()));
			for (TableColumn column : columns) {
				DBLogger.v(SqliteUtility.TAG, String.format("[column = %s, datatype = %s]", column.getColumn(), column.getDataType()));
			}
		}
	}
