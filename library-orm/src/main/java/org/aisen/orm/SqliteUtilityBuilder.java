...
public class SqliteUtilityBuilder {
	...
	public SqliteUtility build(Context context) {
		...
		if (sdcardDb) {
            ...
            DBLogger.d(TAG, String.format(String.format("打开app库 %s, version = %d", dbName, db.getVersion())));
		}
		else {
			db = new SqliteDbHelper(context, dbName, version).getWritableDatabase();

            DBLogger.d(TAG, String.format(String.format("打开sdcard库 %s, version = %d", dbName, db.getVersion())));
		}
	...
	static SQLiteDatabase openSdcardDb(String path, String dbName, int version) {
		...
		if (db != null) {
			...
			DBLogger.d(TAG, "表 %s 的version = %d, newVersion = %d", dbName, dbVersion, version);
