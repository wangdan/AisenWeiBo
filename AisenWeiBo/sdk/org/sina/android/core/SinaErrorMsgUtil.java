...
public class SinaErrorMsgUtil {
	...
	private static final String[][] errorMsgs = new String[][] { { "10002", "服务暂停" }, { "10009", "任务过多，系统繁忙" }, { "10010", "任务超时" }, { "10017", "分组名称长度超过限制" },
			...
			{ "20020", "包含广告信息" }, { "20021", "包含非法内容" }, { "20101", "该微博已经删除" }, { "20031", "需要验证码,操作太频繁" }, { "20104", "不合法的微博" },
	...
	public static TaskException transToException(String responseStr) {
		...
		String errorMsg = GlobalContext.getInstance().getString(R.string.timeout);
			...
			try {
				...
				if (jsonMsg.has("error")) {
					if (jsonMsg.has("error_code")) {
						errorMsg = jsonMsg.getString("error");
						if ("invalid_access_token".equals(errorMsg)) {
							errorCode = "21327";
						}
						else {
							errorCode = jsonMsg.getString("error_code");
						}
		...
		return new TaskException(errorCode, errorMsg);
	}
