...
public class FollowersDB implements ICacheUtility {
	...
	public static void insertFriends(List<WeiBoUser> friendList) {
		SinaDB.getSqlite().insertList(new Extra(AppContext.getUser().getIdstr(), "Followers"), friendList);
	}
	
	public static List<WeiBoUser> selectAll() {
		String selection = String.format(" %s = ? and %s = ? ", FieldUtils.OWNER, FieldUtils.KEY);
		String[] selectionArgs = new String[]{ AppContext.getUser().getIdstr(), "Followers" };
		
		return SinaDB.getSqlite().selectAll(WeiBoUser.class, selection, selectionArgs, null, "200");
	}
	
	public static void clear() {
		SinaDB.getSqlite().deleteAll(new Extra(AppContext.getUser().getIdstr(), "Followers"), WeiBoUser.class);
	}
	...
	public <T> Cache<T> findCacheData(Setting action, Params params, Class<T> responseCls) {
		...
		if (userList.size() > 0) {
			...
			users.setCache(true);
			users.setExpired(CacheTimeUtils.isExpired("Followers", AppContext.getUser()));
			users.setNext_cursor(ActivityHelper.getInstance().getIntShareData("Followers" + AppContext.getUser().getIdstr(), 0));
		...
		if (save) {
			...
			if (users.getUsers().size() > 0) {
				...
				if (params.containsKey("cursor") && Integer.parseInt(params.getParameter("cursor")) == 0) {
					CacheTimeUtils.saveTime("Followers", AppContext.getUser());
					
					clear();
				}
				
				insertFriends(users.getUsers());
				ActivityHelper.getInstance().putIntShareData("Followers" + AppContext.getUser().getIdstr(), users.getNext_cursor());
			}
		}
	}
