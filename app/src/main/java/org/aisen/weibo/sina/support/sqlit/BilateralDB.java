...
public class BilateralDB implements ICacheUtility {
	...
	public static void insertFriends(List<WeiBoUser> friendList) {
		SinaDB.getDB().insert(new Extra(AppContext.getAccount().getUser().getIdstr(), "Bilateral"), friendList);
	}
	
	public static List<WeiBoUser> selectAll() {
		String selection = String.format(" %s = ? and %s = ? ", FieldUtils.OWNER, FieldUtils.KEY);
		String[] selectionArgs = new String[]{ AppContext.getAccount().getUser().getIdstr(), "Bilateral" };
		
		return SinaDB.getDB().select(WeiBoUser.class, selection, selectionArgs, null, null, null, "200");
	}
	
	public static void clear() {
		SinaDB.getDB().deleteAll(new Extra(AppContext.getAccount().getUser().getIdstr(), "Bilateral"), WeiBoUser.class);
	}
