...
public class FollowersDB implements ICacheUtility {
	...
	public static void insertFriends(List<WeiBoUser> friendList) {
		SinaDB.getSqlite().insert(new Extra(AppContext.getUser().getIdstr(), "Followers"), friendList);
	}
	...
	public <T> Cache<T> findCacheData(Setting action, Params params, Class<T> responseCls) {
		...
		if (userList.size() > 0) {
			...
			users.setNext_cursor(ActivityHelper.getIntShareData("Followers" + AppContext.getUser().getIdstr(), 0));
	...
	public void addCacheData(Setting action, Params params, Object responseObj) {
		...
		if (save) {
			...
			if (users.getUsers().size() > 0) {
				...
				ActivityHelper.putIntShareData("Followers" + AppContext.getUser().getIdstr(), users.getNext_cursor());
			}
		}
	}
